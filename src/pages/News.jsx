...
function News() {
  ...
  const handleNewsClick = news => {
    if (!viewedContent.includes(news.title)) {
      const updatedViewedContent = [...viewedContent, news.title];
      setViewedContent(updatedViewedContent);
      localStorage.setItem(
        "viewedContent",
        JSON.stringify(updatedViewedContent)
      );
      Cookie.set("gift", 1);
    }

    window.location.href = news.originallink;
  };
      ...
      <CenterContainer>
        ...
        <NewsContainer>
          {newsData.map((news, index) => (
            <NewsBox key={index} onClick={() => handleNewsClick(news)}>
